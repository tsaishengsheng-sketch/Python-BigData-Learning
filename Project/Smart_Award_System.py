...
功能：智慧年度評選系統 (整合型業務工作流實作)
技術亮點：
    1. 模組化封裝：將 BMI 計算、績效評估、會場規劃與安全投票拆解為獨立函式 (def)，落實「單一職責原則」。
    2. 安全驗證機制：利用 random 生成動態六位數驗證碼，並在投票模組實作核對邏輯，模擬企業安全身分驗證。
    3. 連貫業務邏輯：實作等第門檻判定（GPA A 級限制），僅允許符合條件的資料觸發後續的 math 面積運算與計票流程。
    4. 演算法與數學應用：整合正多邊形面積公式與歐幾里得距離運算，並利用串列 (List) 索引管理動態計票結果。
"""
import math
import random
